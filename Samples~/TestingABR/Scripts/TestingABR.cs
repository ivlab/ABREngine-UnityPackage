...
namespace IVLab.ABREngine.Examples
{
    ...
    public class TestingABR : MonoBehaviour
    {
        void Start()
        {
            ...
            RawDataset dataset = ABREngine.Instance.Data.LoadRawDataset("Demo/Wavelet/KeyData/Contour");

            // Or, work with a VisAsset (this one exists in ABREngine Runtime/Resources/media/visassets):
            IVisAsset visAsset = ABREngine.Instance.VisAssets.LoadVisAsset(new Guid("1af025aa-f1ed-11e9-a623-8c85900fd4af"));

            // You may also get any information from the ABR configuration,
            // such as the default room-scale data bounds or the schema
            // version:
            Bounds defaultBounds = ABREngine.Instance.Config.dataContainer;
            string schema = ABREngine.Instance.Config.SchemaJson["version"].ToString();
